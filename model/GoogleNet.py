...
class Inception(nn.Module):
    def __init__(self, in_channel, channels, bn=True, activation='relu'):
        super().__init__()
        self.channel = np.sum([np.sum(channel) for channel in channels])
        ...
        conv3x3 += [Conv_Block(in_channel, channels[1][1]*2, kernel_size=3, bn=bn, activation=activation)]
        self.conv3x3 = nn.Sequential(*conv3x3)

        conv5x5 = [Conv_Block(in_channel, channels[2][0], kernel_size=1, bn=bn, activation=activation)]
        conv5x5 += [Conv_Block(in_channel, channels[2][1], kernel_size=5, bn=bn, activation=activation)]
        self.conv5x5 = nn.Sequential(*conv5x5)

        max_pool = [nn.MaxPool2d((3,3), 2)]
...
class GoogleNet(nn.Module):
    def __init__(self, channel, n_classes, image_size, in_channel):
        ...
        layers += [nn.Linear(h*w*layers[-4].channel, n_classes)]
